...
Если вы видите `Sync Status: Bootstrap`, это означает что нода Coda загружается и должна синхронизироваться с остальной частью сети. Нода должна получить все необходимые данные, поэтому это займет какое-то время, наберитесь немного терпения. Когда статус синхронизации изменится на `Synced` и нода подключена к одному или более пирам (одноранговым узлам), это означает что мы полностью синхронизировали нашу ноду и подключились к сети. После завершения синхронизации в логах демона появится сообщение `[Info] Coda daemon is now synced`.
...
Поскольку открытый ключ довольно длинный и его трудно запомнить, давайте сохраним его в переменную среды:
